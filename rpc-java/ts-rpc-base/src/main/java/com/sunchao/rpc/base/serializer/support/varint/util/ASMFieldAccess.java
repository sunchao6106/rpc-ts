...
/**
 * Helper class to access the specified class's fields, which is based-on <i>ASM</i>
 * 
 * @author <a href="mailto:sunchao6106@163.com">sunchao</a>
 *
 */
public abstract class ASMFieldAccess {
	...
	@SuppressWarnings("rawtypes")
	public static ASMFieldAccess get(Class type) {
		...
		while (nextClass != Object.class) {
			...
			for (int i = 0, n = declaredFields.length; i < n; i++) {
				...
				if (Modifier.isStatic(modifiers)) continue; //static pass;
				if (Modifier.isPrivate(modifiers)) continue; //private pass;
				fields.add(field);
			}
		...
		if (accessClassName.startsWith("java.") || accessClassName.startsWith("javax.")
				|| accessClassName.startsWith("sun."))
			accessClassName = "reflect." +  accessClassName;
		Class accessClass = null;
		
		ASMClassLoader loader = ASMClassLoader.get(type);
		synchronized (loader) {
			try {
				...
				cw.visit(V1_1, ACC_PUBLIC + ACC_SUPER, accessClassNameInternal, null, "com/sunchao/rpc/base/serializer/support/varint/util/ASMFieldAccess", 
						null);
		...
		try {
			ASMFieldAccess access = (ASMFieldAccess) accessClass.newInstance();
			access.fieldNames = fieldNames;
			access.fieldTypes = fieldTypes;
			return access;
		} catch (Throwable t) {
			throw new RuntimeException("Error when constructing field access class: " + accessClassName, t);
		}
	}
	...
	private static void insertConstructor(ClassWriter cw) {
		...
		mv.visitMethodInsn(INVOKESPECIAL, "com/sunchao/rpc/base/serializer/support/varint/util/ASMFieldAccess", "<init>", "()V");
		mv.visitInsn(RETURN);
		mv.visitMaxs(1, 1);
		mv.visitEnd();
	}
	...
	private static void insertSetPrimitive(ClassWriter cw, String classNameInternal, ArrayList<Field> fields, Type primitiveType) {
		int maxStack = 6;
		int maxLocals = 4;//see correction below for LLAOD and DLOAD
		final String settMethodName;
		final String typeNameInternal = primitiveType.getDescriptor();
		final int loadValueInstruction;
		switch (primitiveType.getSort()) {
		case Type.BOOLEAN:
			settMethodName = "setBoolean";
			loadValueInstruction = ILOAD;
			break;
		case Type.BYTE:
			settMethodName = "setByte";
			loadValueInstruction = ILOAD;
			break;
		case Type.CHAR:
			settMethodName = "setChar";
			loadValueInstruction = ILOAD;
			break;
		case Type.SHORT:
			settMethodName = "setShort";
			loadValueInstruction = ILOAD;
			break;
		case Type.INT:
			settMethodName = "setInt";
			loadValueInstruction = ILOAD;
			break;
		case Type.FLOAT:
			settMethodName = "setFloat";
			loadValueInstruction = FLOAD;
			break;
		case Type.LONG:
			settMethodName = "setLong";
			loadValueInstruction = LLOAD;
			maxLocals++;//LLOAD and DLOAD actually load two slots
			break;
		case Type.DOUBLE:
			settMethodName = "setDouble";
		...
		MethodVisitor mv = cw.visitMethod(ACC_PUBLIC, settMethodName, "(Ljava/lang/Object;I" + typeNameInternal + ")V", null, 
				null);
		mv.visitCode();
		mv.visitVarInsn(ILOAD, 2);
		...
		if (!fields.isEmpty()) {
			...
			//rest of fields: differnent type.
	...
	private static void insertGetPrimitive(ClassWriter cw, String classNameInternal, ArrayList<Field> fields, Type primitiveType) {
		...
		if (!fields.isEmpty()) {
			...
			//reset of fields: different type.
			if (hasAnyBadTypeLabel) {
				mv.visitLabel(labelForInvalidTypes);
				mv.visitFrame(F_SAME, 0, null, 0, null);
				insertThrowExceptionForFieldType(mv, primitiveType.getClassName());
	...
	private static void insertGetString(ClassWriter cw, String classNameInternal, ArrayList<Field> fields) {
		...
		if (!fields.isEmpty()) {
			...
			Label[] labels = new Label[fields.size()];
			Label labelForInvalidTypes = new Label();
			boolean hashAnyBadTypeLabel = false;
			for (int i = 0, n = labels.length; i < n; i++) {
				if (fields.get(i).getType().equals(String.class)) 
					labels[i] = new Label();
				else {
				    labels[i] = labelForInvalidTypes;
				    hashAnyBadTypeLabel = true;
				}
			}
			Label defaultLabel = new Label();
			mv.visitTableSwitchInsn(0, labels.length - 1, defaultLabel, labels);
	...
	private static MethodVisitor insertThrowExceptionForFieldNotFound(
			MethodVisitor mv) {
		...
		mv.visitMethodInsn(INVOKEVIRTUAL, "java/lang/IllegalArgumentException", "<init>", "(Ljava/lang/String;)V");
		mv.visitInsn(ATHROW);
		return mv;
	}
